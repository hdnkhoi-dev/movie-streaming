...
class MovieService{
    ...
    async getMovieList(page, limit, keyword, type){
         ...
         // Search by keyword (title or original_title)
    ...
    async getDiscoverMovies(page, limit, genres, minRating, year, mediaType, country, keyword) {
        ...
        // Search by keyword (title or original_title)
        if (keyword && keyword.trim() !== '') {
            // Fuzzy search logic: "natra" matches "Na Tra", "bat man" matches "Batman"
            // We split the keyword into characters and allow optional spaces between them
            ...
            filter.$or = [
                { title: regex },
                { original_title: regex },
                { name: regex }, // For TV shows sometimes name is used instead of title
                { original_name: regex }
            ];
        ...
        if (genres && genres.length > 0) {
            // genres is array of strings (ids)
            // We want movies that have ALL of these genres (AND logic)
        ...
        // Filter by country (original_language)
    ...
    async addReply(movieId, reviewUserEmail, replyUserEmail, replyUserName, content) {
        ...
        try {
            ...
            // Notification to Tagged User (@Email)
