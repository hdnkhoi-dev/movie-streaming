...
router.post("/insert-movie", async function (req,res) {
    try {
        ...
        if (req.body.genre_ids) {
            // Ensure genre_ids is always an array of strings (ObjectIds)
        ...
        mov.seasons = []; // We store seasons in a separate collection

        await movieService.insertMovie(mov);
        
        // Save seasons if TV show
        ...
        if (mov.media_type === 'tv' && seasonsData && seasonsData.length > 0) {
            ...
            // mov._id is populated by insertOne in the repository
...
router.post("/update-movie", async function(req,res){
    try {
        var movieService = new MovieService();
        var mov = new Movie();
        mov._id = new ObjectId(req.body._id); // Use _id from hidden field
        mov.adult = req.body.adult === 'true' || req.body.adult === true;
        mov.backdrop_path = req.body.backdrop_path;
        mov.media_type = req.body.media_type || 'movie';
        ...
        mov.seasons = []; // Keep it clean in Movie collection
...
// API: Get seasons for a movie (used by admin moviemanage edit modal)
