...
router.get("/", verifyWebToken, async function (req, res) {
    ...
    // Normalize string to match genre names (remove accents, lowercase)
    function normalizeVN(str) {
        return str.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
    }
    
    // Find horror genre with normalized matching
    ...
    if (horrorGenre) {
        // Try to get popular movies with this genre ID
    ...
    // Build rating stats map per item (no bulk API)
    let ratingStatsMap = {};
    try {
        const collect = (arr) => {
